...
class TaskModelTestCase(TestCase):
    def test_create_task1(self):
        ...
        def test_creat_task2(self):
            ...
            task = Task.objects.get(pk='task2')
        ...
        def test_is_overdue_past(self):
            due = timezone.make_aware(datetime(2024, 6, 30, 23, 59, 59))
            current2 = timezone.make_aware(datetime(2024, 7, 1, 0, 0, 0))
            task = Task(title='task1', due_at=due)
            task.save()

            self.assertTrue(tasl.is_overdue(current2))

        def test_is_overdue_none(self):
            due = timezone.make_aware(datetime(2024, 6, 30, 23, 59, 59))
            current3 = timezone.make_aware(datetime(2024, 7, 1, 0, 0, 0))
            task = Task(title='task1', due_at=due)
            task.save()

            self.assertFalse(tasl.is_overdue(current2))
